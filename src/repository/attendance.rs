...
/// Daftar santri satu SESI yang menunggu verifikasi kehadiran.
///
/// `actor_id` Some = dibatasi ke petugas sesi itu (guru pengisi, atau wali
/// kelas bila sesi belum menetapkan pengisi); None = admin/ketua (semua).
///
/// SATU tahap: petugas sesi memutuskan, titik. Syaratnya IDENTIK
/// dengan `decide_verify_bulk` — daftar & aksi untuk hal yang sama tak boleh
/// berbeda, kalau tidak layar menawarkan tombol yang mustahil berhasil.
            ...
            &format!(
                "SELECT a.id, u.full_name, u.nis, cl.name, a.scanned_at, a.gate_label, a.status \
                 ...
                 WHERE a.class_session_id = $1 AND a.verify_status = 'pending' \
                   AND ($2::bigint IS NULL OR ({PETUGAS_SESI_SQL}) = $2) \
                 ORDER BY u.full_name"
            ),
            &[&session_id, &actor_id],
        )
...
/// Antrean tahap final: menunggu verifikasi USTAD bertugas sesi. `teacher_id`
...
/// SIAPA petugas sebuah sesi — satu sumber kebenaran untuk seluruh pintu
/// absensi (daftar antrean, keputusan, dan koreksi).
///
/// Pemanggil wajib menyediakan dua alias: `cs` (`class_sessions`) dan `cl`
/// (`classes` milik sesi itu). Menghasilkan satu `bigint` atau NULL.
///
/// ── ATURANNYA, DAN BERLAKU SAMA UNTUK SEMUA KATEGORI ─────────────────────────
/// Guru yang ditunjuk mengisi sesi itu; bila sesi belum menunjuk siapa pun,
/// jatuh ke WALI KELAS-nya. Kelas non-KBM (piket, sholat, apel) tidak
/// dikecualikan: ia juga punya wali kelas, yang boleh menunjuk guru per sesi
/// atau memverifikasi sendiri ketika sesinya kosong.
///
/// PERIZINAN tidak ikut aturan ini — izin santri selalu diputus wali kelas KBM
/// santri itu, ke mana pun ia menghadiri kegiatan (`kelas_kbm_santri`). Yang di
/// sini soal kehadiran di satu ruangan; yang di sana soal tanggung jawab atas
/// seorang santri.
///
/// ── KENAPA SATU STRING, BUKAN DELAPAN SALINAN ────────────────────────────────
/// Pola yang sama dengan [`DELTA_SQL`] di bawah. Aturan ini dipakai di delapan
/// pernyataan, dan DAFTAR wajib sepakat dengan KEPUTUSAN: kalau menyimpang,
/// seseorang melihat baris yang keputusannya selalu gagal diam-diam, atau
/// memutuskan baris yang tak pernah boleh ia lihat. Kedua hal itu sudah pernah
/// terjadi di berkas ini ketika aturannya masih disalin per query.
const PETUGAS_SESI_SQL: &str = "COALESCE(cs.teacher_id, cl.wali_kelas_id)";
...
/// Verifikasi FINAL oleh ustad bertugas (migrasi 33). Guard: petugas sesi
/// (guru sesi, cadangannya wali kelas) = `actor_id`; None = dewan/admin, yang
/// boleh memutuskan sesi siapa pun. POIN diberikan DI SINI, sekali saja →
/// hindari dobel dengan trigger (migrasi 32). Return true bila ada baris
/// ter-update.
///
/// Daftar dan keputusan HARUS memakai syarat yang sama — lihat
/// [`session_verify_list`] — kalau tidak orang bisa memutuskan baris yang tak
/// pernah boleh ia lihat.
...
pub async fn decide_verify_bulk(
    ...
    let row = c
        .query_one(&sql, &[&att_ids, &approver, &actor_id])
        .await
...
pub async fn decide_verify(
    ...
    let row = tx
        .query_opt(
            ...
            &[&att_id, &status, &approver, &actor_id],
        )
        .await
...
/// yang sudah TUNTAS. Auto-verified (approved oleh sistem)
             ...
             ins AS ( \
                INSERT INTO attendances \
                    (user_id, class_session_id, class_schedule_id, status, method, \
                     verify_status, verified_at, note, gate_label, scanned_at, scan_date) \
                SELECT DISTINCT cp.user_id, s.id, s.class_schedule_id, 'absent', 'manual', \
                       'approved', NOW(), 'Auto: tidak hadir', 'system', NOW(), s.session_date \
...
/// Auto-verifikasi FINAL utk absensi yg menunggu >1 hari (scanned_at lawas).
/// POIN diberikan
...
/// Koreksi status absensi oleh GURU PENGISI sesi itu (cadangannya wali kelas
/// tempat sesi berlangsung) — dan hanya mereka.
///
...
/// konteks.
///
/// Petugasnya ditentukan [`PETUGAS_SESI_SQL`], sama persis dengan pintu
/// verifikasi — mencatat dan mengesahkan adalah wewenang orang yang sama.
///
...
pub async fn correct_attendance(
    ...
    // Penjagaan ADA DI QUERY, bukan cuma di service: pemanggil harus petugas
    // sesi ini (lihat PETUGAS_SESI_SQL).
    let updated = tx
        .execute(
            &format!(
...
/// Petugas sesi sebuah absensi: guru pengisi (atau wali kelasnya bila sesi
/// belum menetapkan pengisi). Dipakai UI untuk menentukan apakah tombol koreksi
/// perlu ditampilkan.
...
/// query: `None` = sesi tak ada ATAU pemanggil bukan guru sesi itu.
///
/// COALESCE ke WALI KELAS: bila sesi belum menetapkan pengisinya, yang berlaku
/// adalah wali kelas — jadi sesi yang terlanjur
